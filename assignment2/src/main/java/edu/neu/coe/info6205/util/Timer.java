...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (preFunction == null) {
                resume();
                postFunction_ = function.apply(supplier.get());
            } else {
                T preFunction_ = preFunction.apply(supplier.get());
                resume();
                postFunction_ = function.apply(preFunction_);
            }

//            resume();
            pauseAndLap();
            if (postFunction != null) {
                postFunction.accept(postFunction_);
            }
        }
